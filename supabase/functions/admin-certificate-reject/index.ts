// Admin: Reject interpreter certificate (requires admin user)
...
Deno.serve(async (req) => {
  try {
    ...
    const update = await svc
      ...
      .update({ is_verified: false, status: "rejected", reviewed_at: new Date().toISOString(), review_note: reject_note ?? null })
      .eq("id", certificate_id)
      .select("id, user_id")
      .maybeSingle();
    if (update.error) throw update.error;

    // Optional: ensure details reflect non-verified
    if (update.data?.user_id) {
      await svc.from("interpreter_details").update({ is_verified: false }).eq("user_id", update.data.user_id);
    }
